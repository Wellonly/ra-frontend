...
import { FieldProps, AppState, Order, Product, ID } from '../types';
    ...
    const { loaded, data: products } = useQueryWithStore(
        {
            ...
            resource: 'products',
            payload: {
                filter: {ids: record ? record.basket.map(item => item.product_id) : []},
            },
        },
        {},
        (state: AppState) => { //TODO: optimize it...
            const productIds = record ? record.basket.map(item => item.product_id): [];
            return productIds
                .map<Product>((productId: ID) => state.admin.resources.products.data[productId] as Product)
            ...
            <Table>
                <TableHead>
                    <TableRow>
                        <TableCell>
                            {translate(
                                'resources.commands.fields.basket.sku'
                            )}
                        </TableCell>
