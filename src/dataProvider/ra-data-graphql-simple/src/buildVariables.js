...
const buildCreateUpdateVariables = (
    ...
    Object.keys(params.data).reduce((acc, key) => {
        if (Array.isArray(params.data[key])) {
            const arg = queryType.args.find(a => a.name === `${key}`); //zv rem: `${key}Ids`

            if (arg) {
                return {
                    ...acc,
                    [`${key}`]: params.data[key].map(ed => ed), /* zv rem: `${key}Ids` and map(({ id }) => id) */
                };
            }
        }

        if (typeof params.data[key] === 'object') {
            const arg = queryType.args.find(a => a.name === `${key}`); //zv rem: `${key}Id`

            if (arg) {
                return {
                    ...acc,
                    [`${key}`]: params.data[key]?.id || null, /* zv rem: `${key}Id`; zv add: || null */
                };
            }
        }
...
export default introspectionResults => (
    ...
    switch (aorFetchType) {
        ...
        case GET_MANY_REFERENCE: {
            ...
            variables.filter[`${parts[0]}`] = preparedParams.id; // zv rem: variables.filter[`${parts[0]}Id`] = preparedParams.id;
            return variables;
        }
        case GET_ONE:
        case DELETE:
